...
function [c,ns] = poly_der_fit(x0,x1,x2,y0,y1,y2,v0,v2,degree)
    assert(degree >= 4);
    A = zeros(degree+1);
    
    ind = degree:-1:0;
    A(1,:) = ones(1,degree+1)*diag(x0.^ind);
    A(2,:) = ones(1,degree+1)*diag(x1.^ind);
    A(3,:) = ones(1,degree+1)*diag(x2.^ind);
    
    pd = polyder(ones(1,degree+1));
    ind = degree-1:-1:0;
    A(4,1:degree) = pd * diag(x0.^ind);
    A(5,1:degree) = pd * diag(x2.^ind);
